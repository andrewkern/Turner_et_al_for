...
*/
double omegaFromTo(struct sequenceMatrix *aSeqMat, int start, int stop, int site){
	int i,j, segsites, s;
	vector *locs;
	...
	for(i=0; i<segsites-1; i++){
		for(j=i+1; j<segsites; j++){
			comp = dij(aSeqMat,vectorGetIndex(locs,i),vectorGetIndex(locs,j));
	...
	numer *= (sumL+sumR);
//	printf("d: %f n %f sumL: %f sumR: %f term: %f left: %d\n",denom,numer,sumL,sumR,1.0 / ((site*(site-1)/2) + ((s-site)*(s-site-1)/2)),site);
	return(numer/denom);
}
...
double omegaAtCenter(struct sequenceMatrix *aSeqMat, int start, int stop, double site){
	int i,j, segsites, s, min, siteIdx, loc_i, loc_j;
	vector *locs;
...
double omegaMaxFromTo(struct sequenceMatrix *aSeqMat, int start, int stop){
	...
	if(segsites < 2)
		return(0);
	for(l=2;l<segsites-2;l++){
		tmp = omegaFromTo(aSeqMat,start,stop,l);
		if(tmp > max) max = tmp;
	}
	return(max);
}
...
double siteAssoc(struct sequenceMatrix *aSeqMat, int indexA, int indexB){
	char majAlleleA, majAlleleB;
	int pA, pB, pAB, pa, pb, pab, pAb, paB;
	int comp,i;
	double freqA, freqB;
